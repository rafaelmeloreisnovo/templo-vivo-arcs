...
def is_safe_fill(field: str, old, new) -> bool:
    ...
    if old == new:
        return True
    # A date-only historical observation may be enriched by a verified UTC timestamp.
...
def main() -> int:
    ...
    errors: list[str] = []
    applied = 0
    uncertainty_by_target: dict[str, list[str]] = {}
    ...
    for lineno, res in resolution_rows:
        ...
        uncertainty_by_target[target] = res["remaining_uncertainty"]
        applied += 1
